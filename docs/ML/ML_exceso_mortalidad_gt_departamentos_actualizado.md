# Clasificación de Exceso de Mortalidad en Guatemala por Departamento mediante Regresión Logística

## Nombre del problema

**Clasificación binaria de exceso de mortalidad en Guatemala por departamento y mes.**

El objetivo del modelo es identificar si un departamento de Guatemala presenta **exceso de mortalidad** en un mes específico, comparando las muertes observadas contra una línea base histórica pre-COVID.
...
| Archivo | Descripción |
|---|---|
| `ml_exceso_mortalidad_gt_departamentos.ipynb` | Notebook de Databricks utilizado para conectarse al Data Warehouse, consultar defunciones generales de Guatemala, construir la variable objetivo, entrenar el modelo, evaluar métricas, exportar el `.tar.gz` y registrar el modelo en MLflow. |
| `modelo_exceso_mortalidad_gt_departamentos_logreg.tar.gz` | Paquete comprimido con el modelo entrenado y los artefactos necesarios para documentar, evaluar y reutilizar el modelo. |
...
| Tabla | Uso |
|---|---|
| `dw.fact_defunciones_gt` | Tabla de hechos con defunciones generales de Guatemala. |
| `dw.dim_tiempo` | Dimensión temporal usada para obtener año y mes. |
| `dw.dim_geografia_gt` | Dimensión geográfica usada para obtener departamento y municipio. |

La fact contiene información mensual entre 2018 y 2024. Para este modelo se agrupan las defunciones a nivel departamento, año y mes.

El grano del dataset es:
...
| Departamento | Año | Mes | Muertes observadas |
|---|---:|---:|---:|
| Guatemala | 2024 | 6 | 1200 |
| Quetzaltenango | 2024 | 6 | 350 |
| Alta Verapaz | 2024 | 6 | 280 |

---

## Construcción de la variable objetivo

Primero se calcula una línea base histórica pre-COVID usando los años **2018 y 2019**.
...
```text
Promedio histórico de Guatemala en enero = promedio de enero 2018 y enero 2019.
Promedio histórico de Quetzaltenango en enero = promedio de enero 2018 y enero 2019.
```
...
En otras palabras, si las muertes observadas superan en al menos 15% el promedio histórico pre-COVID del mismo departamento y mes, la observación se etiqueta como exceso de mortalidad.
...
|---|---|---|
| `departamento` | Categórica | Departamento de Guatemala. |
| `anio` | Numérica | Año de la observación. |
| `mes` | Numérica | Mes de la observación. |
| `promedio_pre_covid` | Numérica | Promedio histórico pre-COVID del departamento y mes. |
| `desviacion_pre_covid` | Numérica | Desviación histórica pre-COVID del departamento y mes. |
...
3. Consulta `dw.fact_defunciones_gt` junto con las dimensiones de tiempo y geografía.
4. Agrupa las defunciones por departamento, año y mes.
5. Convierte los datos a pandas para el procesamiento de Machine Learning.
6. Calcula la línea base histórica pre-COVID 2018-2019 por departamento y mes.
7. Calcula el porcentaje de exceso de mortalidad.
8. Crea la variable objetivo `exceso_mortalidad`.
9. Genera visualizaciones por departamento.
10. Divide los datos en entrenamiento y prueba.
11. Entrena una Regresión Logística con scikit-learn.
12. Evalúa el modelo con métricas de clasificación.
13. Compara distintos umbrales de decisión.
14. Selecciona el mejor umbral según F1-score.
15. Exporta el modelo en un archivo `.tar.gz`.
16. Registra el modelo y artefactos en MLflow.
...
|---|---|---|
| Categórica | `departamento` | `OneHotEncoder(handle_unknown="ignore")` |
| Numéricas | `anio`, `mes`, `promedio_pre_covid`, `desviacion_pre_covid` | `StandardScaler()` |

`OneHotEncoder` convierte el nombre del departamento en columnas numéricas. `StandardScaler` normaliza las variables numéricas para que el modelo no se vea dominado por variables con escalas más grandes.
...
```text
modelo_exceso_mortalidad_gt_departamentos_logreg.tar.gz
```
...
| Archivo | Descripción |
|---|---|
| `modelo_logreg_defunciones_gt.joblib` | Modelo entrenado. Contiene el pipeline completo: preprocesamiento y Regresión Logística. |
| `feature_columns.joblib` | Lista de columnas que el modelo espera recibir para predecir. |
| `baseline_pre_covid.csv` | Línea base histórica pre-COVID 2018-2019 por `departamento` y `mes`. Permite obtener automáticamente `promedio_pre_covid` y `desviacion_pre_covid`. |
| `config_modelo.json` | Configuración del problema, variable objetivo, umbral de decisión y features usadas. |
...
El siguiente ejemplo recibe únicamente departamento, año y mes. Luego busca automáticamente el promedio y la desviación en `baseline_pre_covid.csv`.
...
tar_path = "modelo_exceso_mortalidad_gt_departamentos_logreg.tar.gz"
extract_dir = "modelo_gt_extraido"

with tarfile.open(tar_path, "r:gz") as tar:
    tar.extractall(extract_dir)

# 2. Cargar artefactos
modelo = joblib.load(os.path.join(extract_dir, "modelo_logreg_defunciones_gt.joblib"))
...
with open(os.path.join(extract_dir, "config_modelo.json"), "r", encoding="utf-8") as f:
    config = json.load(f)
...
# No se escriben manualmente promedio_pre_covid ni desviacion_pre_covid.
...
fila_baseline = baseline[
    (baseline["departamento"].astype(str).str.upper() == departamento_objetivo.upper()) &
    (baseline["mes"].astype(int) == int(mes_objetivo))
]

if fila_baseline.empty:
    raise ValueError("No existe línea base histórica para ese departamento y mes.")
...
umbral = config.get("umbral_decision_modelo", metricas.get("umbral_decision", 0.55))
prediccion = int(probabilidad >= umbral)

print("Departamento:", departamento_objetivo)
...
### Promedios del `classification_report`

| Salida | Significado |
|---|---|
| `macro avg` | Promedio simple entre clases. Da el mismo peso a la clase 0 y a la clase 1. |
| `weighted avg` | Promedio ponderado por cantidad de registros de cada clase. Las clases con más datos pesan más. |
| `support` | Cantidad de registros reales de cada clase. |
...
| Modelo | Pipeline completo de scikit-learn o modelo PyFunc, según la celda usada para el registro. |

Si se registra como PyFunc, el modelo puede devolver directamente:

| Salida | Descripción |
|---|---|
| `probabilidad_exceso_mortalidad` | Probabilidad de que la observación pertenezca a la clase 1. |
| `prediccion_exceso_mortalidad` | Clase final usando el umbral configurado. |
...
La línea base pre-COVID se calcula con 2018-2019 debido a la disponibilidad de datos en `dw.fact_defunciones_gt`. Una ventana histórica más amplia podría mejorar la estabilidad de la línea base, pero para esta prueba de concepto los años 2018 y 2019 permiten construir una referencia previa a la pandemia.

El modelo trabaja a nivel departamento y mes. Aunque la dimensión geográfica contiene municipios, se agrupa a nivel departamento para reducir dispersión y mejorar estabilidad del modelo.

---

## Conclusión

Este modelo permite clasificar exceso de mortalidad en Guatemala a nivel departamento-mes. Utiliza PySpark para conectarse al Data Warehouse y scikit-learn para entrenar una Regresión Logística con función sigmoide, Log Loss, regularización L2 y pesos balanceados por clase. El paquete `.tar.gz` incluye el modelo, métricas, comparación de umbrales y la línea base histórica necesaria para utilizar el modelo sin ingresar manualmente `promedio_pre_covid` ni `desviacion_pre_covid`.
