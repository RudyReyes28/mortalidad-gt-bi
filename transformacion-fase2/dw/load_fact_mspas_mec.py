...
y carga los registros en dw.fact_defunciones en ambos destinos:
    - DW local   (DW_DB_URL)
    - DW en nube (DW_CLOUD_URL) — opcional
...
def _clasificar_grupo_etario_mspas(rango: str) -> str:
    ...
    if "mes" in r or "< 1" in r:
        return "< 1 anio"
    if any(x in r for x in ["1 a", "2 a", "3 a", "4 a"]):
        return "1-4"
    if any(x in r for x in ["5 a","6 a","7 a","8 a","9 a","10 a","11 a","12 a","13 a","14 a"]):
        return "5-14"
    if any(x in r for x in ["15 a","16 a","17 a","18 a","19 a","20 a","21 a","22 a","23 a","24 a","25 a","26 a","27 a","28 a","29 a"]):
        return "15-29"
    if any(x in r for x in ["30 a","31 a","32 a","33 a","34 a","35 a","36 a","37 a","38 a","39 a","40 a","41 a","42 a","43 a","44 a"]):
        return "30-44"
    if any(x in r for x in ["45 a","46 a","47 a","48 a","49 a","50 a","51 a","52 a","53 a","54 a","55 a","56 a","57 a","58 a","59 a"]):
        return "45-59"
    if "60" in r or "65" in r or "70" in r or "75" in r or "80" in r or "+" in r:
        return "60 o mas"
    return "No especificado"
...
def load_fact_mspas_mec(sandbox_url: str, dw_local_url: str, dw_cloud_url: str = None):
...
if __name__ == "__main__":
    ...
    load_fact_mspas_mec(sandbox_url, dw_local_url, dw_cloud_url)