...
Lee stage.stage_defunciones_gt, resuelve los IDs de cada dimension
y carga los registros en dw.fact_defunciones.

Nota: Este script fue desarrollado para la fuente INE por el equipo.
Columnas relevantes de stage_defunciones_gt:
    anio_ocurrencia, mes_ocurrencia, nombre_depto_ocurrencia,
    nombre_muni_ocurrencia, sexo, grupo_etario, codigo_cie10, periodo
...
def load_fact_ine(sandbox_url: str, dw_url: str):
    ...
    # 1. Leer Stage — agregar por dimensiones para reducir volumen
    ...
    print_log(f"  -> {len(df):,} filas agregadas (de registros individuales INE).")
...
if __name__ == "__main__":
    ...
    load_fact_ine(sandbox_url, dw_url)